...
@isolate_cat
def seed(points: list[list[int]], session: str = "") -> MaskState:
...
@isolate_cat
def connect(session: str = "") -> MaskState:
...
@isolate_cat
def outline(epsilon: float = 2.0, session: str = "") -> MaskState:
    ...
    poly = ops.Geometry.simplify_contour(ws.current_array(), epsilon)
    if len(poly) < 3:
...
@isolate_cat
def isolate(session: str = "") -> IsolateResult:
...
@isolate_cat
def cutout(rect: list[int] = [], iterations: int = 5, session: str = "") -> IsolateResult:
...
@isolate_cat
def clear_seeds(session: str = "") -> MaskState:
